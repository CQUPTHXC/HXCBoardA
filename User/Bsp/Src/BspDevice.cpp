 ...
 *        - `true`: 设备正在被使用
 */
bool deviceRefCount[DEVICE_COUNT] = {0}; // 引用数组
...
{
  [DEVICE_TIMER_6] = &htim6,
  [DEVICE_TIMER_7] = &htim7,
  [DEVICE_PWM_12] = &htim12,
  [DEVICE_PWM_4] = &htim4,
  [DEVICE_CAN_1] = &hcan1,
  [DEVICE_CAN_2] = &hcan2,
  [DEVICE_SPI_1] = &hspi1,
  [DEVICE_USART_4] = &huart4,
  [DEVICE_USART_6] = &huart6,
  [DEVICE_USART_7] = &huart7,
}; // 设备句柄数组
                                                                             
BspResult<DeviceStatus> Bsp_GetDeviceStatus(BspDevice_t deviceID)
{
  BSP_CHECK(deviceID >= DEVICE_NONE && deviceID < DEVICE_COUNT, BspError::InvalidDevice, DeviceStatus);

  DeviceStatus status;
  status.deviceID = deviceID;
  status.hasHandle = (deviceHandles[deviceID] != nullptr);
  status.isUse = deviceRefCount[deviceID];
  status.halHandle = deviceHandles[deviceID];

  return BspResult<DeviceStatus>::success(status);
}
